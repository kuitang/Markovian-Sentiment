...
static int do_init(void)
{
    ...
    /* Verb example sentences only in versin 1.6 */

    if (!strcmp(wnrelease, "1.6")) {
	sprintf(tmpbuf,VRBSENTFILE,searchdir);
	if ((vsentfilefp = fopen(tmpbuf, "r")) == NULL) {
	    sprintf(msgbuf,
"WordNet library error: Can't open verb example sentence file(%s)\n",
		    tmpbuf);
	    display_message(msgbuf);
	    openerr = -1;
	}
	...
	if ((vidxfilefp = fopen(tmpbuf, "r")) == NULL) {
	    sprintf(msgbuf,
"WordNet library error: Can't open verb example sentence index file(%s)\n",
		    tmpbuf);
	    display_message(msgbuf);
	    openerr = -1;
	}
